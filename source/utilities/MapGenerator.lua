...
-- with that level+roomNumber exists. See RunState.startDebugRoom / the debug scene menu.
...
function MapGenerator.selfCheck(progress)
	...
	-- reachability (BFS from start). Follow both door edges and portal links, since
	-- secret rooms are linked to their host only via portals (PortalID), never via a
	-- cardinal door edge — but they are still reachable from the start room.
	...
	while #queue > 0 do
		...
		if graph[id].portals then
			for _, destId in pairs(graph[id].portals) do
				if not seen[destId] then seen[destId] = true; table.insert(queue, destId) end
			end
		end
	end
