-- entities/UI/MapDrawer.lua
-- Renders the current procedural run graph (RunState.graph) using love.graphics in the
-- 400x240 virtual canvas space. The caller is responsible for setting the target canvas
-- before calling MapDrawer.draw(); this module issues love.graphics calls directly.
--
-- Style mirrors the old fixed-floor map: filled squares with a marker for the current room.
-- Unvisited boxes/edges are drawn at Config.Map.unvisitedAlpha (Playdate used a dither).
--
...
local function buildLayout(g)
    ...
    if minCol == math.huge then return cells, 0, 0, portalLinks end
...
-- Draw the current run graph onto the active love.graphics canvas in virtual coords.
function MapDrawer.draw()
    ...
    local unvisAlpha   = Config.Map.unvisitedAlpha
    ...
    love.graphics.setLineWidth(Config.Map.lineWidth)
    for id = 1, #g do
        local node = g[id]
        local cell = cells[id]
        if node and cell and node.edges then
            for _, d in ipairs(DIRS) do
                ...
                if neighborCell and id < nid then
                    ...
                    love.graphics.setColor(roomColor[1], roomColor[2], roomColor[3], a)
    ...
    -- Portal links (secret room -> its host): always at unvisitedAlpha, so a portal reads as
    -- a linked secret room rather than a duplicate box floating next to the predecessor.
    for _, link in ipairs(portalLinks) do
        local sc, hc = cells[link.secret], cells[link.host]
        if sc and hc then
            love.graphics.setColor(roomColor[1], roomColor[2], roomColor[3], unvisAlpha)
    ...
    for id = 1, #g do
        ...
        if node and cell then
            ...
            if id == RunState.currentNodeId then
                ...
                love.graphics.setColor(roomColor[1], roomColor[2], roomColor[3], 1)
                love.graphics.rectangle("fill", bx, by, boxSize, boxSize)
                local innerSize = boxSize - 2
                if innerSize < 1 then innerSize = 1 end
                local inset = math.floor((boxSize - innerSize) / 2)
                love.graphics.setColor(markerColor[1], markerColor[2], markerColor[3], 1)
                love.graphics.rectangle("fill", bx + inset, by + inset, innerSize, innerSize)
            elseif node.visited then
                -- Visited room: solid roomColor box.
                love.graphics.setColor(roomColor[1], roomColor[2], roomColor[3], 1)
                love.graphics.rectangle("fill", bx, by, boxSize, boxSize)
            else
                -- In-graph, not visited: roomColor box at unvisitedAlpha.
                love.graphics.setColor(roomColor[1], roomColor[2], roomColor[3], unvisAlpha)
                love.graphics.rectangle("fill", bx, by, boxSize, boxSize)
            end
        end
