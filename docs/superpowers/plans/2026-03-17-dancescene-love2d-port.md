...
**Goal:** Port the Playdate DanceScene rhythm-combat system to a self-contained Love2D project (`love2d-dancescene/`) at the repo root, replicating all gameplay mechanics from the Playdate original.
...
> **Note on libraries:** `middleclass.lua` and `anim8.lua` already exist at `source/libraries/` in this repo. Copy them instead of downloading.
...
- [ ] **Step 3: Copy `middleclass.lua` and `anim8.lua` from `source/libraries/` (already in repo)**

```bash
cp source/libraries/middleclass.lua love2d-dancescene/lib/middleclass.lua
cp source/libraries/anim8.lua love2d-dancescene/lib/anim8.lua
```
...
- [ ] **Step 1: Create `PlayerData.lua`** — mirrors the fields from `source/assets/data/PlayerDataTables.lua` used by DanceScene. Edit values to test different difficulty scenarios.
...
- `PlayerData.isDancing`, `PlayerData.saveLevel`, `PlayerData.lastEnemyTouched` must be set by gameScene before transitioning to DanceScene (same contract as Playdate version).
- The full PlayerData schema lives in `source/assets/data/PlayerDataTables.lua` — that file is the authoritative source of truth for all field names and defaults.
